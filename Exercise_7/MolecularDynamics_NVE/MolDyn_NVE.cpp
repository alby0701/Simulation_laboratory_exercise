...
void Input(std::string inputfile) { // Prepare all stuff for the simulation
    ...
    //std::cout << bin_size << endl;
    // clear output files
    ofstream output;
    output.open("output.dat", ios::trunc);
    output.close();
    output.open("output.gofr.0", ios::trunc);
    output.close();
    output.open("output.gave.0", ios::trunc);
    output.close();
}
...
void measure(){
    double V(0), V1(0);
    //Reset instogram
    for (int k=0; k<(int)nbins; ++k) walker[k]=0.0;
...
void Accumulate_gdir()  {
    for(int i=0; i<(int)nbins; ++i)  blk_av[i] = blk_av[i] + walker[i];
     blk_norm = blk_norm + 1.0;
}
void MeasureBlock_gdir(int block) {
  double gdir;
  ofstream Gofr, Gave;
...
//g(r)
  for (int i=0; i<(int)nbins; ++i) {
    ...
    err_gdir = sqrt((glob_av2[i]/(double)block - pow(glob_av[i]/(double)block,2))/(double)(block-1));
    Gofr << block << "   " <<  i+1 << "   " << gdir << "   " << bin_size*(i)  << "   " << (i+1)*bin_size  <<  endl;
    if(block == (int)nblocks-1)
    Gave << block << "   " <<  i+1 << "   " << glob_av[i]/(double)block << "   " << err_gdir << "   " << bin_size*(i)  << "   " << (i+1)*bin_size  <<  endl;
  }
...
void SaveMeasure(int block) { // Save properties measurement
