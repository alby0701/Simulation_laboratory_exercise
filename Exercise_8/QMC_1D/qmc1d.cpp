...
{
	double k_elastic = 1;
	return k_elastic*(val*val*val*val) - (5.0*val*val)/2.0;
}

double external_potential_prime(double val)
{
	double k_elastic =1;
	return k_elastic*4.0*(val*val*val) - 5.0*val;
}

double external_potential_second(double val)
{
	double k_elastic=1;
	return k_elastic*12.0*(val*val) - 5.0;
}
...
double variationalWaveFunction(double v)
{
  ...
  {
    double mu = 0.8;
    double sigma2 = 0.65*0.65;
    double exp_minus = exp(-pow((v-mu),2)/(sigma2));
    double exp_plus = exp(-pow((v+mu),2)/(sigma2));
...
double variationalWaveFunction_second(double v)
{
  if(function == false)
  ...
  else
   {
     ...
     double exp_minus = exp(-pow((v-mu),2)/(sigma2));
     double exp_plus = exp(-pow((v+mu),2)/(sigma2));
     return -exp_minus/sigma2 - exp_plus/sigma2 + pow((v-mu),2)*exp_minus/pow(sigma2,2) + pow((v+mu),2)*exp_plus/pow(sigma2,2);
   }
...
void readInput()
{
  ...
  input_file >> string_away >> function;
	input_file.close();
	delete [] string_away;
}
